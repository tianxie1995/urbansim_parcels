...
def population_within_15_min(costar, zones):
    return misc.reindex(zones.population_within_15_min,
                        costar.zone_id).fillna(0)
...
@orca.column('assessor_transactions', 'msa_id', cache=True,
...
@orca.column('assessor_transactions', 'is_peripheral', cache=True,
             cache_scope='iteration')
def is_peripheral(assessor_transactions):
    peripheral = ((assessor_transactions.msa_id == 6)
                  | (assessor_transactions.luz_id.isin([155, 156])))
    return peripheral.astype('int')


@orca.column('assessor_transactions', 'jobs_within_30_min')
def jobs_within_30_min(assessor_transactions, zones):
    return misc.reindex(zones.jobs_within_30_min,
                        assessor_transactions.zone_id).fillna(0)
...
def job_spaces():
    ...
    merged = pd.merge(b.reset_index(), bsqft_job,
    ...
    merged['job_spaces'] = np.ceil(
        merged.non_residential_sqft / merged.sqft_per_emp)
    job_spaces = pd.Series(merged.job_spaces, index=b.index)
    b['job_spaces'] = job_spaces
    b.job_spaces[(b.luz_id < 17) & (b.year_built < 2013)] = np.ceil(
        b.job_spaces[(b.luz_id < 17) & (b.year_built < 2013)] / 10.0)
    b.job_spaces[(b.job_spaces > 2000) & (b.year_built < 2013)] = 2000
    b.job_spaces[b.job_spaces.isnull()] = np.ceil(
        b.non_residential_sqft / 200.0)
    b.job_spaces[b.year_built < 2013] = np.ceil(
        b.job_spaces[b.year_built < 2013] / 3.25)
    return b.job_spaces
...
@orca.column('buildings', 'population_within_30_min', cache=True,
             cache_scope='step')
def population_within_30_min(buildings, zones):
    return misc.reindex(zones.population_within_30_min,
                        buildings.zone_id).fillna(0)


@orca.column('buildings', 'population_within_15_min', cache=True,
             cache_scope='step')
...
@orca.column('buildings', 'building_type_id', cache=True,
             cache_scope='iteration')
...
@orca.column('households', 'luz_id_households', cache=True,
             cache_scope='iteration')
...
@orca.column('households', 'income_halves', cache=True,
             cache_scope='iteration')
...
@orca.column('parcels', 'population_within_30_min', cache=True,
             cache_scope='step')
def population_within_30_min(parcels, zones):
    return misc.reindex(
        zones.population_within_30_min, parcels.taz_id).fillna(0)
...
def proportion_developable(parcels):
    parcels = parcels.to_frame(
        columns=['development_type_id', 'proportion_undevelopable'])
    # Set right-of-way parcels as undevelopable
    parcels.proportion_undevelopable[
        parcels.development_type_id == 24] = 1.0
...
def parcel_is_allowed(form):
    ...
    if form == 'residential':
        allowed = (zoning_allowed_uses[19]
                   | zoning_allowed_uses[20]
                   | zoning_allowed_uses[21])
        ...
        allowed = ((zoning_allowed_uses[4] | zoning_allowed_uses[5])
                   & (zoning_allowed_uses[19] | zoning_allowed_uses[20]
                      | zoning_allowed_uses[21]))
...
def parcel_sales_price_sqft(use):
    ...
    if use == "residential":
        ...
        print('Residential price adjustment factor is: %s'
...
def parcel_average_price(use):
    if use == "residential":
        ...
        s = misc.reindex(buildings.
                         res_price_per_sqft[
                             buildings.general_type == "Residential"].
...
# Placeholder-  building height currently unconstrained
# (very high limit-  1000 ft.)
...
def total_sqft(parcels, buildings):
    return buildings.building_sqft.groupby(
        buildings.parcel_id).sum().reindex(parcels.index).fillna(0)


@orca.column('parcels', 'building_purchase_price_sqft', cache=True,
             cache_scope='iteration')
...
@orca.column('parcels', 'building_purchase_price', cache=True,
             cache_scope='iteration')
...
def land_cost(parcels):
    return (parcels.building_purchase_price
...
def total_sfd_du(parcels, buildings):
    ...
    return buildings[
        buildings.development_type_id == 19].residential_units.groupby(
        buildings.parcel_id).sum().reindex(parcels.index).fillna(0)
...
def total_sfa_du(parcels, buildings):
    buildings = buildings.to_frame(buildings.local_columns)
    return buildings[
        buildings.development_type_id == 20].residential_units.groupby(
        buildings.parcel_id).sum().reindex(parcels.index).fillna(0)
...
def total_mfr_du(parcels, buildings):
    buildings = buildings.to_frame(buildings.local_columns)
    return buildings[
        buildings.development_type_id == 21].residential_units.groupby(
        buildings.parcel_id).sum().reindex(parcels.index).fillna(0)


@orca.column('parcels', 'oldest_building')
def oldest_building(parcels, buildings):
    return buildings.year_built.groupby(
        buildings.parcel_id).min().reindex(parcels.index).fillna(9999)


@orca.column('parcels', 'newest_building')
def newest_building(parcels, buildings):
    return buildings.year_built.groupby(
        buildings.parcel_id).max().reindex(parcels.index).fillna(0)
...
def jobs_within_30_min(jobs, travel_data, zone_ids):
    ...
    return misc.compute_range(travel_data,
                              jobs.groupby('zone_id').size().reindex(
                                  index=zone_ids).fillna(0),
                              "am_single_vehicle_to_work_travel_time",
                              30, agg=np.sum)


@orca.column('zones', 'population_within_30_min', cache=True,
             cache_scope='step')
def population_within_30_min(households, travel_data, zone_ids):
    travel_data = travel_data.to_frame()
    households = households.to_frame(columns=['persons', 'zone_id'])
    return misc.compute_range(travel_data, households.groupby(
        'zone_id').persons.sum().reindex(index=zone_ids).fillna(0),
                              "am_single_vehicle_to_work_travel_time",
                              30, agg=np.sum)


@orca.column('zones', 'population_within_15_min', cache=True,
             cache_scope='step')
