...
def buildings_to_uc(new_buildings, year):
    ...
    if ((new_buildings.index.name != 'building_id')
            & ('building_id' not in new_buildings.columns)):
        print('Index of buildings must be "building_id" or "building_id"'
              'column must exist. Skipping export-to-Urban-Canvas.')
        return None
    ...
    if len(conn_string) == 0:
        print('A "conn_string" injectable must be registered and populated.'
              'Skipping export-to-Urban-Canvas.')
        return None
    ...
    max_bid = get_val_from_uc_db("select max(building_id) FROM "
                                 "building where building_id<100000000;")
    new_buildings.building_id = np.arange(max_bid + 1,
                                          max_bid + 1 + len(new_buildings))

    if 'projects_num' not in orca.list_injectables():
        exec_sql_uc("INSERT INTO scenario(id, name, type) select "
                    "nextval('scenario_id_seq'), "
                    "'Run #' || cast(currval('scenario_id_seq') "
                    "as character varying), 1;")
        nextval = get_val_from_uc_db("SELECT MAX(ID) FROM SCENARIO "
                                     "WHERE ID < 100000;")
        orca.add_injectable('projects_num', nextval)

        exec_sql_uc("INSERT INTO scenario_project(scenario, project) "
                    "VALUES(%s, 1);" % nextval)
        exec_sql_uc("INSERT INTO scenario_project(scenario, project) "
                    "VALUES(%s, %s);" % (nextval, nextval))
    ...
    print('Exporting %s buildings to Urban Canvas'
          'database for project %s and year %s.' % (len(new_buildings),
                                                    nextval,
                                                    year))
    output = cStringIO.StringIO()
    new_buildings.to_csv(output, sep='\t', header=False, index=False)
    output.seek(0)
    cur.copy_from(output, 'building',
...
def get_development_projects():
    ...
    devproj_query = ("select id, placetype_id as building_type_id, "
                     "duration, buildings_number, average_floors as stories, "
                     "sqft as non_residential_sqft, "
                     "sqft_unit as sqft_per_unit, "
                     "units as residential_units, "
                     "Name as name, "
                     "start_date from developmentprojects "
    ...
    dp_pcl_query = ("select developmentprojects_parcels.development_project, "
                    "developmentprojects_parcels.parcel_id, "
                    "parcel.parcel_acres "
                    "from developmentprojects_parcels, parcel where "
                    "developmentprojects_parcels.parcel_id = "
                    "parcel.parcel_id;")
    ...
    scheduled_development_events = pd.merge(devproject_parcel_ids, devproj,
                                            left_on='development_project',
                                            right_on='id')
    scheduled_development_events = scheduled_development_events.rename(
        columns={'development_project': 'scheduled_development_event_id',
