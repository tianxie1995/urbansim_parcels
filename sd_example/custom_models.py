...
def households_transition(households, annual_household_control_totals, year):
    ...
    new, added_hh_idx, empty_dict = model.transition(hh, year, )
...
def model_integration_indicators():
    ...
    mgra_indicators = hh.groupby(
        ['mgra_id', 'activity_id']).size().reset_index()
    mgra_indicators.columns = ['mgra_id', 'activity_id',
...
def buildings_to_uc(buildings, settings):
    if settings['urbancanvas']:
        # Export newly predicted buildings
        # (from proforma or scheduled_development_events) to Urban Canvas
        from sd_example import urbancanvas
        year = get_year()
        b = buildings.to_frame(buildings.local_columns)
        ...
        new_buildings['building_sqft'] = (new_buildings.residential_sqft
                                          + new_buildings.non_residential_sqft)
        new_buildings['sqft_per_unit'] = (new_buildings.residential_sqft
                                          / new_buildings.residential_units)

        # These are the fields you need.
        # And index should be named 'building_id'.
        new_buildings = new_buildings[['parcel_id', 'building_type_id',
                                       'improvement_value',
                                       'residential_units',
                                       'non_residential_sqft', 'stories',
                                       'year_built', 'building_sqft',
                                       'sqft_per_unit']]

        for col in ['parcel_id', 'residential_units', 'non_residential_sqft',
                    'year_built', 'stories', 'building_sqft', 'sqft_per_unit',
                    'improvement_value']:
...
def add_extra_columns(df):
    ...
    df.development_type_id[(df.form == 'residential')
                           & (df.dua >= 12) & (df.dua < 25)] = 20
    df.development_type_id[(df.form == 'residential')
                           & (df.dua >= 26)] = 21
...
def luz_indicators():
    bsim = orca.get_table('buildings').to_frame(
        columns=['luz_id', 'note', 'res_price_per_sqft',
                 'nonres_rent_per_sqft', 'residential_units',
                 'non_residential_sqft'])
    luz_res_price = bsim[
        bsim.residential_units > 0].groupby('luz_id').res_price_per_sqft.mean()
    luz_nonres_price = bsim[
        bsim.non_residential_sqft > 0].groupby(
        'luz_id').nonres_rent_per_sqft.mean()
    bsim = bsim[(bsim.note == 'simulated')
                | bsim.note.str.startswith('Sitespec')]
    luz_simdu = bsim.groupby('luz_id').residential_units.sum()
    luz_simnr = bsim.groupby('luz_id').non_residential_sqft.sum()
    luz_df = pd.DataFrame(
        {'du': luz_simdu, 'nrsf': luz_simnr, 'res_price': luz_res_price,
         'nonres_price': luz_nonres_price})
    ...
    luz_df['hh_diff'] = (hh_sim.groupby('luz_id').size().fillna(0)
                         - luz_base_indicators.hh_base.fillna(0))
    luz_df['emp_diff'] = (emp_sim.groupby('luz_id').size().fillna(0)
                          - luz_base_indicators.emp_base.fillna(0))
...
def msa_indicators():
    ...
    b = orca.get_table('buildings').to_frame(
        columns=['msa_id', 'mgra_id', 'residential_units',
                 'non_residential_sqft', 'note'])
