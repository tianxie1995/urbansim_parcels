...
def run_new_feasibility(parcels, parcel_price_callback,
                        parcel_use_allowed_callback,
                        parcel_occupancy_callback=None, start_year=None,
                        cfg=None, **kwargs):
    """
    ...
    df = new_prepare_parcels_for_feasibility(sites, parcel_price_callback,
                                             pf, parcel_occupancy_callback,
                                             start_year)
    feasibility = lookup_by_form(df, parcel_use_allowed_callback, pf, **kwargs)
    orca.add_table('feasibility', feasibility)
...
def new_prepare_parcels_for_feasibility(sites, parcel_price_callback,
                                        pf, parcel_occupancy_callback=None,
                                        start_year=None):
    """
    Prepare parcel DataFrame for feasibility analysis
...
def new_run_developer(forms, agents, buildings, supply_fname, feasibility,
    ...
    new_sites = (
        new_process_new_buildings(dev.feasibility, buildings, new_buildings,
                                  form_to_btype_callback,
                                  add_more_columns_callback,
                                  supply_fname, remove_developed_buildings,
                                  unplace_agents))

    add_new_units(dev, new_sites)
...
def new_process_new_buildings(feasibility, buildings, new_buildings,
                              form_to_btype_callback,
                              add_more_columns_callback,
                              supply_fname, remove_developed_buildings,
                              unplace_agents):
...
def build_from_pipeline(pipeline, dev_projects, dev_sites, year):
    ...
    all_buildings = merge_buildings(old_buildings, ds_this_year)
    orca.add_table('buildings', all_buildings)
